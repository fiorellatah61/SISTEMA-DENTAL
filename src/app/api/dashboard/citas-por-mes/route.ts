...
// NUEVO --------------------------
// ========================================
// src/app/api/dashboard/citas-por-mes/route.ts
// ========================================
import { NextResponse } from 'next/server'
...
export async function GET() {
  try {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const citasDelMes = await executeWithRetry(() =>
        prisma.cita.count({
          where: {
            fechaHora: {
              gte: inicioMes,
            ...
            estado: {
              in: ['CONFIRMADA', 'MODIFICADA']
            }
          }
        })
      );

      const ingresosDelMes = await executeWithRetry(() =>
      ...
      mesesData.push({
        mes: nombresMeses[fecha.getMonth()],
        citas: citasDelMes || 0,
        ingresos: (ingresosDelMes && Number(ingresosDelMes._sum.monto)) || 0
      });
    ...
    return NextResponse.json(mesesVacios)
  }
}