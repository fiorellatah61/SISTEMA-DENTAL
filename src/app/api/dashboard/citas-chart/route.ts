...
// Propósito: API para obtener datos de citas por día (últimos 7 días) para el gráfico LineChart.
// Usa cache para evitar consultas repetidas y maneja errores con datos por defecto.
// CORREGIDO: Usa ORM de Prisma en lugar de SQL raw para evitar errores de nombres de tablas
...
export async function GET() {
  try {
    ...
    // Consulta optimizada usando ORM de Prisma (más seguro que SQL raw)
    const startDate = dates[0];
    const endDate = new Date(dates[6].getTime() + 24 * 60 * 60 * 1000);

    const citasRaw = await executeDashboardQuery(
      async () => {
        // Usar ORM de Prisma en lugar de SQL raw
    ...
    // Procesar datos agrupados por fecha
    const citasMap = new Map<string, number>();
    citasRaw.forEach(cita => {
      const dateKey = cita.fechaHora.toDateString();
      citasMap.set(dateKey, (citasMap.get(dateKey) || 0) + 1);
    });
