...
/// NUEVO -----------------------------
// ========================================
// src/app/api/dashboard/pacientes-por-edad/route.ts
// ========================================
import { NextResponse } from 'next/server'
...
export async function GET() {
  try {
    ...
    pacientes.forEach((pacienteItem: any) => {
      const edad = pacienteItem.edad || (pacienteItem.fechaNacimiento ? calcularEdad(pacienteItem.fechaNacimiento) : null)
    ...
    const totalPacientes = pacientes.length
    const pacientesPorEdad = Object.entries(rangosEdad)
      .map(([rango, datos]) => ({
        rango,
        cantidad: datos.cantidad,
        porcentaje: totalPacientes > 0 ? 
    ...
    return NextResponse.json(pacientesPorEdad)
