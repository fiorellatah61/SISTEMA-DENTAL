...
//           {/* Contenido del Tab 5 - Odontograma */}
//           <div className="p-6">
...
export default function OdontogramaPage() {
  ...
  const renderizarSimboloProcedimiento = (numeroDiente: number) => {
    ...
    return (
      <div 
        className="absolute inset-0 flex items-center justify-center text-white font-bold pointer-events-none z-10 text-xs sm:text-sm"
        style={{
          textShadow: '1px 1px 2px rgba(0,0,0,0.8)'
        }}
      >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-4 sm:space-y-0">
            <div>
              <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">CLÍNICA DENTAL SONRÍE</h1>
            </div>
            <div className="flex items-center space-x-4">
              <div className="text-sm text-gray-600">
                N° de Ficha: 
        ...
        <div className="bg-white rounded-lg shadow-sm border mb-6 p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
            ...
            <button
              onClick={buscarPaciente}
              disabled={loading}
              className="bg-gray-800 text-white px-4 py-2 rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 disabled:opacity-50 w-full sm:w-auto sm:mt-6"
            >
              {loading ? 'Buscando...' : 'Buscar'}
            </button>
            <div className="text-sm text-gray-600 sm:mt-6 w-full sm:w-auto">
              Paciente: {pacienteSeleccionado ? `${pacienteSeleccionado.nombres} ${pacienteSeleccionado.apellidos}` : 'No seleccionado'}
            </div>
          </div>
          {error && (
        ...
        <div className="bg-white rounded-lg shadow-sm border mb-6">
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6">
              {/* Odontograma principal */}
              <div className="lg:col-span-3">
                <div className="border border-blue-200 rounded-lg p-3 sm:p-4 bg-blue-50 shadow-sm overflow-hidden">
                  {/* Dientes superiores */}
                  <div className="mb-6 sm:mb-8">
                    <h3 className="text-sm font-medium mb-3 sm:mb-4 text-center text-blue-800">Dientes Superiores</h3>
                    <div className="flex justify-center space-x-0.5 sm:space-x-1 overflow-x-auto pb-2">
                      {(mostrarTemporales ? DIENTES_TEMPORALES.superiores : DIENTES_PERMANENTES.superiores).map((numeroDiente) => (
                        <button
                          key={numeroDiente}
                          onClick={() => handleDienteClick(numeroDiente)}
                          className="relative group hover:scale-105 transition-transform duration-200 flex-shrink-0"
                          title={`Diente ${numeroDiente}`}
                        >
                          <div className="relative w-8 h-12 sm:w-10 sm:h-14 md:w-12 md:h-16">
                            {/* Fondo de color para el procedimiento */}
                            <div 
                              className="absolute inset-0 rounded opacity-70 shadow-inner"
                              style={{
                                backgroundColor: obtenerColorFondoDiente(numeroDiente)
                              }}
                            ></div>

                            {/* Imagen del diente con diseño adicional */}
                            <Image
                              src={`/images/dientes/${numeroDiente}.png`}
                              alt={`Diente ${numeroDiente}`}
                              width={48}
                              height={64}
                              className="relative z-5 w-full h-full object-contain opacity-90 filter drop-shadow-md"
                              onError={(e) => {
                                const target = e.target as HTMLImageElement;
                                target.style.display = 'none';
                                target.nextElementSibling?.classList.remove('hidden');
                            ...
                            {/* Fallback - mostrar número si no hay imagen */}
                            <div className="hidden absolute inset-0 bg-blue-100 border border-blue-300 rounded flex items-center justify-center text-xs font-semibold z-5">
                              {numeroDiente}
                            </div>
                  ...
                  {/* Dientes inferiores */}
                  <div>
                    <h3 className="text-sm font-medium mb-3 sm:mb-4 text-center text-blue-800">Dientes Inferiores</h3>
                    <div className="flex justify-center space-x-0.5 sm:space-x-1 overflow-x-auto pb-2">
                      {(mostrarTemporales ? DIENTES_TEMPORALES.inferiores : DIENTES_PERMANENTES.inferiores).map((numeroDiente) => (
                        <button
                          key={numeroDiente}
                        ...
                        >
                          <div className="relative w-8 h-12 sm:w-10 sm:h-14 md:w-12 md:h-16">
                            ...
                            <Image
                              ...
                              width={48}
                              height={64}
                              className="relative z-5 w-full h-full object-contain transform rotate-180 opacity-90 filter drop-shadow-md"
                              onError={(e) => {
                                const target = e.target as HTMLImageElement;
                                target.style.display = 'none';
                                target.nextElementSibling?.classList.remove('hidden');
              ...
              <div className="space-y-4 sm:space-y-6">
                {/* Leyenda de Procedimientos */}
                <div className="border border-blue-200 rounded-lg p-3 sm:p-4 bg-blue-50 shadow-sm">
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="text-sm font-semibold text-blue-800">Leyenda de Procedimientos</h3>
                    <button
                      onClick={() => window.open('/dashboard/procedimientos', '_blank')}
                  ...
                  <div className="space-y-1 max-h-40 overflow-y-auto pr-2">
                    {procedimientos.slice(0, 8).map((proc, index) => (
                      <div key={proc.id} className="flex items-center text-xs">
                        <span className="w-4 mr-2">{index + 1}</span>
                        <span className="flex-1 truncate">{proc.nombre}</span>
                        <div className="w-8 h-4 rounded flex items-center justify-center text-white text-xs mr-1 flex-shrink-0" style={{backgroundColor: proc.color}}>
                          {proc.simbolo}
                        </div>
                      </div>
                    ))}
                ...
                {/* Leyenda de Condiciones */}
                <div className="border border-blue-200 rounded-lg p-3 sm:p-4 bg-blue-50 shadow-sm">
                  <h3 className="text-sm font-semibold mb-2 text-blue-800">Condiciones Actuales</h3>
                  <div className="space-y-1 max-h-40 overflow-y-auto pr-2">
                    {condiciones.slice(0, 5).map((cond, index) => (
                      <div key={cond.id} className="flex items-center text-xs">
                        <span className="w-4 mr-2">{index + 1}</span>
                        <span className="flex-1 truncate">{cond.nombre}</span>
                        <div className="w-8 h-4 rounded flex items-center justify-center text-white text-xs mr-1 flex-shrink-0" style={{backgroundColor: cond.color}}>
                          {cond.simbolo}
                        </div>
                      </div>
                    ))}
                ...
                {/* Estados */}
                <div className="border border-blue-200 rounded-lg p-3 sm:p-4 bg-blue-50 shadow-sm">
                  <h3 className="text-sm font-semibold mb-2 text-blue-800">ESTADOS</h3>
                  <div className="space-y-1">
                    <div className="flex items-center text-xs">
                    ...
                    <div className="flex items-center text-xs">
                      <div className="w-3 h-3 rounded-full bg-blue-800 mr-2 flex-shrink-0"></div>
                      <span>Necesita atención</span>
                    </div>
                    <div className="flex items-center text-xs">
                  ...
                  <table className="min-w-full table-auto border-collapse">
                    <thead>
                      <tr className="bg-blue-50">
                        <th className="border border-blue-200 px-2 sm:px-4 py-2 text-left text-sm font-medium text-blue-800">FECHA</th>
                        <th className="border border-blue-200 px-2 sm:px-4 py-2 text-left text-sm font-medium text-blue-800">N° DE DIENTES</th>
                        <th className="border border-blue-200 px-2 sm:px-4 py-2 text-left text-sm font-medium text-blue-800">CONDICIÓN ACTUAL</th>
                        <th className="border border-blue-200 px-2 sm:px-4 py-2 text-left text-sm font-medium text-blue-800">PROCEDIMIENTO</th>
                        <th className="border border-blue-200 px-2 sm:px-4 py-2 text-left text-sm font-medium text-blue-800">SUBTIPO</th>
                        <th className="border border-blue-200 px-2 sm:px-4 py-2 text-left text-sm font-medium text-blue-800">CARA DENTAL</th>
                        <th className="border border-blue-200 px-2 sm:px-4 py-2 text-left text-sm font-medium text-blue-800">ESPECIFICACIONES</th>
                        <th className="border border-blue-200 px-2 sm:px-4 py-2 text-left text-sm font-medium text-blue-800">ESTADO</th>
                      </tr>
                    </thead>
                    <tbody>
                      {odontograma.piezasOdontograma
                        ...
                        .map((pieza) => (
                        <tr key={pieza.id}>
                          <td className="border border-blue-200 px-2 sm:px-4 py-2 text-sm">
                            {new Date(pieza.fechaRegistro).toLocaleDateString()}
                          </td>
                          <td className="border border-blue-200 px-2 sm:px-4 py-2 text-sm">{pieza.diente}</td>
                          <td className="border border-blue-200 px-2 sm:px-4 py-2 text-sm">{pieza.condiciones || '-'}</td>
                          <td className="border border-blue-200 px-2 sm:px-4 py-2 text-sm">{pieza.procedimiento}</td>
                          <td className="border border-blue-200 px-2 sm:px-4 py-2 text-sm">{pieza.subtipo || '-'}</td>
                          <td className="border border-blue-200 px-2 sm:px-4 py-2 text-sm">{pieza.caraDental || '-'}</td>
                          <td className="border border-blue-200 px-2 sm:px-4 py-2 text-sm">{pieza.especificaciones || '-'}</td>
                          <td className="border border-blue-200 px-2 sm:px-4 py-2 text-sm">
                            <span className={`px-2 py-1 rounded-full text-xs ${
                              pieza.estado === 'Saludable' ? 'bg-green-100 text-green-800' :
                              pieza.estado === 'Seguimiento' ? 'bg-yellow-100 text-yellow-800' :
