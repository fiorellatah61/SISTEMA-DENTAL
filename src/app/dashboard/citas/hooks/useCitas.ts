...
export function useCitas() {
  ...
  // MEJORADO: Solo esta función para los recordatorios
  const enviarRecordatorios = async () => {
    setLoading(true)
    setError('')
    try {
      ...
      const controller = new AbortController()
      const timeoutId = setTimeout(() => controller.abort(), 30000) // Más tiempo para recordatorios
      ...
      if (data.success) {
        ...
        return {
          success: false,
          error: data.error || 'Error desconocido al enviar recordatorios'
        }
      }
    } catch (error: any) {
      if (error.name === 'AbortError') {
